...
initial_train_size = int(len(subset_feed_data) * 0.8)  # 初始訓練集大小
validation_size = int(len(subset_feed_data) * 0.2)     # 驗證集大小
...
count = 0
for data_per_batch_number in subset_feed_data[initial_train_size:]:
    ...
    encoder = OneHotEncoder(categories=categories, handle_unknown='ignore', sparse=False)
    print(df[['Feed_Type']])
    print(df['Feed_Type'].dtype)
    print(df['Feed_Type'].unique())  # 打印唯一值，看看是否有异常值
    # 進行編碼
    encoded_features = encoder.fit_transform(df[['Feed_Type']])
